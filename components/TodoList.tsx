...
const TodoList: React.FC<TodoListProps> = ({ todos, setTodos, onOpenSettings, lang, t }) => {
  ...
  // ✅ FINAL Delete method with Supabase support
  ...
  const handleGenerate = async () => {
    ...
    setKeyError(false);
    try {
      const jsonStr = await generateTodoSuggestions("I am a retail apprentice (V-Markt). Suggest typical tasks for Betrieb (e.g. MHD control, Cashier) and Berufsschule.");
  ...
  const getCategoryIcon = (cat: string) => {
    switch (cat) {
      case 'Betrieb': return <Briefcase size={14} />;
      case 'Berufsschule': return <GraduationCap size={14} />;
  ...
  const getCategoryColor = (cat: string) => {
    switch (cat) {
      case 'Betrieb': return 'bg-green-100 text-green-700';
      case 'Berufsschule': return 'bg-blue-100 text-blue-700';
  ...
  const formatDate = (dateStr: string) => {
    ...
    return date.toLocaleDateString(lang === 'de' ? 'de-DE' : 'en-US', { day: '2-digit', month: '2-digit' });
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      {/* Input Area */}
      <form onSubmit={addTodo} className="bg-white p-2 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row gap-2">
        <input 
          type="text" 
          value={newTodoText}
        ...
        <div className="flex gap-2 p-2 md:p-0 items-center">
          <div className="relative">
            <input 
              ...
              className="px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 focus:outline-none focus:border-green-500"
            />
          </div>
          <select 
            value={category}
            onChange={(e) => setCategory(e.target.value as any)}
            className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm font-medium text-slate-600 focus:outline-none focus:border-green-500"
          >
          ...
          <button 
            type="submit"
            className="bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-xl font-medium transition-colors flex items-center gap-2"
          >
            <Plus size={18} />
          ...
          filteredTodos.map((todo) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className={`text-base font-medium truncate ${todo.completed ? 'text-slate-400 line-through' : 'text-slate-800'}`}>
                  {todo.text}
                </p>
                <div className="flex items-center gap-2 mt-1">
                   <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-semibold ${getCategoryColor(todo.category)}`}>
                   ...
                   {todo.completedAt && (
                     <span className="flex items-center gap-1 text-xs text-green-600">
                       <Check size={12} />
                       Done {new Date(todo.completedAt).toLocaleDateString(lang === 'de' ? 'de-DE' : 'en-US')}
                     </span>
                   )}
                </div>
              </div>
